...
namespace WebApplication1
{
    public partial class RequestITReceiving : System.Web.UI.Page
    {
        ...
        public void BindVendor(int Search, string mode)
        {
            try
            {
                List<GetVendorByFacilityID_Result> lstvendordetails = new List<GetVendorByFacilityID_Result>();
                if (Search == 1)
                {
        ...
        public void SearchGrid()
        {
            try
            {
                ...
                List<SearchITReceiving_Result> lstMSRMaster = lclsservice.SearchITReceiving(lstITRe).ToList();
        ...
        protected void lbitrno_Click(object sender, EventArgs e)
        {
            if (defaultPage.RoleID == 1)
            {
                ...
                List<BindITNNOvalues_Result> lstitnnno = lclsservice.BindITNNOvalues(Convert.ToInt64(gvrow.Cells[17].Text)).ToList();
        ...
        public void Reviewfields()
        {
            ...
            {
                List<BindITNNOvalues_Result> lstitnnno = lclsservice.BindITNNOvalues(Convert.ToInt64(ViewState["ITReceivingMasterID"])).ToList();
                txtinvoiceno.Text = lstitnnno[0].InvoiceNo;
                if (lstitnnno[0].InvoiceDate != null)
                    txtinvoicedate.Text = Convert.ToDateTime(lstitnnno[0].InvoiceDate).ToString("MM/dd/yyyy");
                if (lstitnnno[0].ReceivingAction != null)
        ...
        public void SaveITReceiving()
        {
            ...
            List<UpdateITRecevingMaster_Result> msgrecngmaster = new List<UpdateITRecevingMaster_Result>();
        ...
        protected void imgsummary_Click(object sender, ImageClickEventArgs e)
        {
            ...
            ITRONo = lbitrno.Text;
            ITNNO = lbitono.Text;
            List<BindITReceivingsummaryReport_Result> llstreview = lclsservice.BindITReceivingsummaryReport(ITNNO, Convert.ToInt64(defaultPage.UserId), "").ToList();
        ...
        protected void imgdetails_Click(object sender, ImageClickEventArgs e)
        {
            ...
            ImageButton btndetails = sender as ImageButton;
            GridViewRow gvrow = (GridViewRow)btndetails.NamingContainer;
            List<BindITReceivingDetailsReport_Result> llstreview = lclsservice.BindITReceivingDetailsReport(Convert.ToInt64(gvrow.Cells[2].Text), Convert.ToInt64(defaultPage.UserId), "").ToList();
