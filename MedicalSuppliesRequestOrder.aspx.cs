    ...
    public partial class MedicalSuppliesRequestOrder : System.Web.UI.Page
    {
        ...
        public void BindGrid()
        {
            try
            {
                ...
                List<SearchMedicalSupplyRequestPo_Result> lstMSRMaster = lclsservice.SearchMedicalSupplyRequestPo(llstMSRSearch).ToList();
            ...
            try
            {
                ...
                if (ReviewCount != 0)
                {
                    ...
                    if (meditem == "Saved Successfully")
                    {
                        ...
                        btnorderall.Text = "Order All";
                        List<GetMSRMultipleIDs_Result> lstrwpo = lclsservice.GetMSRMultipleIDs(Convert.ToString(ViewState["PRMatserID"])).ToList();
        ...
        public List<object> DetailsOrderReport(Int64 PRmasterID)
        {
            ...
            List<GetMSROrderContentPO_Result> llstreview = lclsservice.GetMSROrderContentPO(PRmasterID, defaultPage.UserId).ToList();
        ...
        public List<object> SearchOrderReport(string PRmasterID)
        {
            string smedmasterIds = string.Empty;
            List<object> llstarg = new List<object>();
            List<GetMedicalSupplyPoReportDetails_Result> llstreview = new List<GetMedicalSupplyPoReportDetails_Result>();
            if (PRmasterID == "")
            {
                foreach (GridViewRow row in grdOrderSearch.Rows)
                {
            ...
            rvMSRPoReport.LocalReport.ReportPath = Server.MapPath("~/Reports/MedicalSuppliesPOReport.rdlc");
            ...
            this.rvMSRPoReport.LocalReport.SetParameters(p1);
            ReportDataSource datasource = new ReportDataSource("MSRPoReportDS", llstreview);
            ...
            string extension;
            byte[] bytes = null;
            bytes = rvMSRPoReport.LocalReport.Render("PDF", "", out mimeType, out encoding, out extension, out streamids, out warnings);
        ...
        protected void imgsearchprint_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                List<object> llstresult = SearchOrderReport(MedreqID);
        ...
        public void PurchaseItemDetails(string PRmasterID)
        {
            try
            {
                List<GetMedicalSupplyPoReportDetails_Result> llstreview = lclsservice.GetMedicalSupplyPoReportDetails(null, PRmasterID, defaultPage.UserId, defaultPage.UserId).ToList();
        ...
        protected void gvmedreview_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Image imgrwreadmoreaudit = (Image)e.Row.FindControl("imgrwreadmoreaudit");
