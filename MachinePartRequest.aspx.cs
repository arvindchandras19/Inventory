    ...
    public partial class MachinePartRequest : System.Web.UI.Page
    {
        ...
        public void SearchGrid()
        {
            try
            {
                ...
                List<SearchMPRMaster_Result> lstMSRMaster = lclsservice.SearchMPRMaster(llstMachineSearch).ToList();
        ...
        public void bindreview()
        {
            try
            {
                ...
                else
                {
                    DivMPRMasterNoreview.Style.Add("display", "block");
                    lblmprreview.Visible = true;
                    List<GetMPRMaster_Result> llstmpr = lclsservice.GetMPRMaster().Where(a => a.MPRMasterID == Convert.ToInt64(HddMasterID.Value)).ToList();
        ...
        public void BindGrid(int TestEdit)
        {
            try
            {
                if (TestEdit == 1)
                {
                    ...
                    llstMachineSearch.loggedinBy = defaultPage.UserId;
                    List<SearchMPRMaster_Result> lstMPRMaster = lclsservice.SearchMPRMaster(llstMachineSearch).ToList();
        ...
        public void BindValuesOfQueryString()
        {
            ...
            List<GetMPRDetailsbyMPRMasterID_Result> lstMPRMasterDetails = lclsservice.GetMPRDetailsbyMPRMasterID(Convert.ToInt64(HddMasterID.Value), defaultPage.UserId, Convert.ToInt64(LockTimeOut)).ToList();
        ...
        {
            List<GetMPRMaster_Result> lstMPRMaster = lclsservice.GetMPRMaster().Where(a => a.MPRMasterID == Convert.ToInt64(HddQueryStringID.Value)).ToList();
            BindCorporate(0, "Edit");
        ...
        public void GetEquipmentSubCategory()
        {
            ...
            {
                InventoryServiceClient lclsservice = new InventoryServiceClient();
                List<GetEquipementSubCategory_Result> lstequipsub = lclsservice.GetEquipementSubCategory(Convert.ToInt64(ddlEquipmentCategory.SelectedValue), "Add").ToList();
        ...
        public void GetEquipementList()
        {
            ...
            {
                InventoryServiceClient lclsservice = new InventoryServiceClient();
                List<GetEquipementList_Result> lstequiplist = lclsservice.GetEquipementList(Convert.ToInt64(ddlEquipmentSubCat.SelectedValue), "Add").ToList();
        ...
        public void BindEquipcategory(Int64 CorporateID)
        {
            ...
            {
                InventoryServiceClient lclsservice = new InventoryServiceClient();
                List<GetEquipmentCategory_Result> lstequipcat = lclsservice.GetEquipmentCategory(CorporateID, "Add").ToList();
        ...
        public List<object> SearchOrderReport(string MPRMasterID)
        {
            ...
            List<BindMachinePartsReport_Result> llstreview = new List<BindMachinePartsReport_Result>();
            if (MPRMasterID == "")
            {
                foreach (GridViewRow row in grdMPRMaster.Rows)
                {
        ...
        public void BindDetailGrid(Int64 MPRMasterID, Int64 UserId)
        {
            try
            {
                string LockTimeOut = "";
                LockTimeOut = System.Configuration.ConfigurationManager.AppSettings["LockTimeOut"].ToString();
                List<GetMPRDetailsbyMPRMasterID_Result> llstMPRMaster = lclsservice.GetMPRDetailsbyMPRMasterID(MPRMasterID, UserId, Convert.ToInt64(LockTimeOut)).ToList();
