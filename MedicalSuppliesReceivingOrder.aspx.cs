...
namespace WebApplication1
{
    public partial class MedicalSuppliesReceivingOrder : System.Web.UI.Page
    {
        ...
        //        List<SearchMedicalSuppliesReceiving_Result> lstMSRMaster = lclsservice.SearchMedicalSuppliesReceiving(llstMSRSearch).ToList();
        ...
        public void BindDetailGrid(Int64 PRmarsterID)
        {
            try
            {
                ...
                List<BindMedicalsupplyReceivingDetail_Result> lstMSRMaster = lclsservice.BindMedicalsupplyReceivingDetail(llstMSRDeatilItem).ToList();
        ...
        protected void grdReciveSearch_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Image imgsummaryprint = (Image)e.Row.FindControl("imgsummaryprint");
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<ValidateMedicalSuppliesOrder_Result> llstmedicaldetail = lclsservice.ValidateMedicalSuppliesOrder(objmedsupplyreceive.PRMasterID).ToList();
                ...
                if (ErrorList == "")
                {
                    List<UpdateMSRReceivingMaster_Result> lstMSRMaster = lclsservice.UpdateMSRReceivingMaster(objmedsupplyreceive).ToList();
        ...
        protected void imgsummaryprint_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                List<GetMSRReceivingsummaryReport_Result> llstreview = lclsservice.GetMSRReceivingsummaryReport(objmedsupplyreceive).ToList();
        ...
        protected void imgdetailprint_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                objmedsupplyreceive.LoggedinBy = defaultPage.UserId;
                objmedsupplyreceive.SearchFilters = "";
                List<BindMedicalSupplyDetailsReport_Result> llstreview = lclsservice.BindMedicalSupplyDetailsReport(objmedsupplyreceive).ToList();
                ...
                string encoding;
                string extension;
                byte[] bytes = rvdetailreport.LocalReport.Render("PDF", "", out mimeType, out encoding, out extension, out streamids, out warnings);
                Guid guid = Guid.NewGuid();
                string path = ConfigurationManager.AppSettings["TempFileLocation"].ToString();
                _sessionPDFFileName = "MSReceivingOrder" + guid + ".pdf";
                if (!Directory.Exists(path)) Directory.CreateDirectory(path);
